...
var _aTemplate2 = _interopRequireDefault(require("a-template"));
...
var aTable = /*#__PURE__*/function (_aTemplate) {
  _inherits(aTable, _aTemplate);

  var _super = _createSuper(aTable);
  ...
  function aTable(ele, option) {
    ...
    _this.addTemplate(_this.menu_id, _util["default"].removeIndentNewline(menu));

    _this.data = (0, _deepExtend["default"])({}, defs, option);
    var data = _this.data;
    ...
    data.row = _this.parse("<table>".concat(selector.innerHTML, "</table>"));
  ...
  _createClass(aTable, [{
    ...
    value: function _getElementsByQuery(query) {
      return document.querySelectorAll("[data-id='".concat(this.id, "'] ").concat(query));
    }
  }, {
    key: "_getSelf",
    ...
    value: function getCellByIndex(x, y) {
      return this._getElementByQuery("[data-cell-id='".concat(x, "-").concat(y, "']"));
    }
  }, {
    ...
    value: function getCellInfoByIndex(x, y) {
      ...
      var pos = _util["default"].offset(cell);
      ...
      [].forEach.call(headers, function (header, index) {
        if (_util["default"].offset(header).left === left) {
          returnLeft = index;
        }
      });
      [].forEach.call(sides, function (side, index) {
        if (_util["default"].offset(side).top === top) {
          returnTop = index;
        }
      });
      return {
    ...
    value: function parse(html) {
      ...
      var doc = _util["default"].parseHTML(html);
    ...
    value: function parseText(text) {
      var arr1 = []; // replace newline codes inside double quotes to <br> tag
    ...
    value: function getTableClass(html) {
      return _util["default"].parseHTML(html).getAttribute('class');
    }
  }, {
    key: "toMarkdown",
    value: function toMarkdown(html) {
      var table = _util["default"].parseHTML(html);
    ...
    value: function onUpdated() {
      ...
      if (table) {
        ...
        inner.style.width = "".concat(tableWidth, "px");
      } else {
    ...
    value: function undo() {
      ...
      if (row) {
        if (hist.length === 0) {
          hist.push((0, _clone["default"])(row));
        }
    ...
    value: function insertRow(a, newrow) {
      var data = this.data;
      var row = data.row; // TODO: move below condition in override method
      // code start
    ...
    value: function removeCol(selectedno) {
      var data = this.data;
      data.showMenu = false;
      var self = this; // TODO: move below condition in override method
      // code start
    ...
    value: function updateTable(b, a) {
      ...
      if (type === 'compositionstart') {
        ...
        _util["default"].triggerEvent(elem, 'copy');
      } else if (type === 'copy') {
        this.copyTable(e);
      } else if (type === 'paste') {
        this.pasteTable(e);
        ...
        if (_util["default"].hasClass(this.e.target, 'a-table-editable') && this.e.target.parentNode.getAttribute('data-cell-id') === "".concat(b, "-").concat(a)) {
          data.history.push((0, _clone["default"])(data.row));
    ...
    value: function copyTable(e) {
      ...
      this.data.row.forEach(function (item, i) {
        ...
        item.col.forEach(function (obj, t) {
          if (obj.selected) {
            copy_text += "<".concat(obj.type, " colspan=\"").concat(obj.colspan, "\" rowspan=\"").concat(obj.rowspan, "\">").concat(obj.value, "</").concat(obj.type, ">");
          }
        });
        copy_text += '</tr>';
      });
    ...
    value: function insertTable(table, pos) {
      ...
      var rows = [];
      var data = this.data;
      var prevRow = (0, _clone["default"])(data.row);
      ...
      rows.forEach(function (row) {
        ...
        if (table[t]) {
          table[t].col.reverse().forEach(function (cell) {
            _this4.insertCellAt(index.row, index.col + 1, {
              type: 'td',
    ...
    value: function insertColRight(selectedno) {
      var data = this.data; // TODO: move below condition in override method
      // code start
    ...
    value: function insertRowAbove(selectedno) {
      ...
      data.history.push((0, _clone["default"])(this.data.row));
      this.update();
    }
  }, {
    key: "mergeCells",
    ...
    value: function splitCell() {
      ...
      var bound = {
        x: 0,
        y: selectedPoint.y,
        width: selectedPoint.x,
        height: selectedPoint.height
      };
      var points = this.getAllPoints();
      ...
      rows.forEach(function (row) {
        ...
        for (var _i7 = 0; _i7 < width; _i7++) {
          ...
          self.insertCellAt(index.row, index.col + 1, {
            type: 'td',
            colspan: 1,
            rowspan: 1,
            value: val,
    ...
    key: "changeCellToBlank",
    value: function changeCellToBlank() {
      var data = this.data;
      data.row.forEach(function (item, i) {
        item.col.forEach(function (obj, t) {
          if (obj.selected) {
